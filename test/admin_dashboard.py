...
#callback to check for correct userid and allow access
@app.callback(
    Output("dynamic-layout", "children"),
    Input("url", "search"),
    prevent_initial_call=False 
)
def route_based_on_id(search):
    from urllib.parse import parse_qs

    if not search:
        return html.Div("❌ No ID provided in URL.")

    query_params = parse_qs(search.lstrip("?"))
    user_id = query_params.get("user_id", [""])[0]

    if user_id.lower() == "admin":  # You can change 'admin' to whatever your admin ID is
        return get_admin_layout()
    else:
        return html.Div("⛔ Access denied. Admins only.")
